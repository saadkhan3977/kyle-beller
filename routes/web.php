...
Route::get('/artisan', function () {
   ...
   $command = 'php artisan storage:link';
    ...
    // You can access the output of the command if needed.
  return  $output = Artisan::output();

    // Handle the result as needed.
    if ($exitCode === 0) {
        return "Artisan command '{$command}' executed successfully.";
    } else {
        return "Artisan command '{$command}' failed with exit code {$exitCode}. Output: {$output}";
    }
});
...
Route::get('/export-database', function () {
    function backupDatabaseAllTables($dbhost,$dbusername,$dbpassword,$dbname,$tables = '*'){
        ...
        request()->session()->flash('success','Database Export Successfully');
...
Route::get('get_variant/{id}',[FrontendController::class,'get_variant']);
...
// Cart section
Route::post('/add-to-cart/{slug}',[\App\Http\Controllers\CartController::class,'addToCart'])->name('add-to-cart')->middleware('user');
...
Route::group(['prefix'=>'admin','middleware'=>['auth','admin']],function(){
	...
	Route::get('variant_delete/{id}',[\App\Http\Controllers\ProductController::class,'deletevariant'])->name('review.store');
    Route::get('/product_search',[\App\Http\Controllers\ProductController::class,'productSearch'])->name('products.search');
